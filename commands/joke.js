...
module.exports.run = async (context, delim) => {
    const arr = Array("Падает комп с виндой с 16-го этажа и думает: \"Вот сейчас бы зависнуть\"",
        ...
        "Отец, рассказывавший сказку сыну, так сильно хотел спать, что колобка ещё до встречи с зайцем сбила фура.",
        "В хороших руках и хуй балалайка.",
        "Преступник хотел ограбить секс-шоп, но ему там дали пизды.",
        "Если Бог создал все на свете, то скорее всего он был китайцем...");

    context.reply(arr[Math.floor(Math.random() * arr.length - 1)])
};
